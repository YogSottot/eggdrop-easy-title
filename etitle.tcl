...
# Указываем канальный флаг(.chanset #chan +etitle для включения скрипта).
setudef flag etitle
...
###                            ###
      ...
      # Список ников чьи урлы обрабатываться не будут.
      # List of ignored nicks.
      set etitle(denynicks) "lamestbot quiz info"

      # Список слов тайтлы с которыми будут игнорироваться.
      ...
      # Используем TinyURL сокращение. Урлы длиннее этого числа будут сокращаться.
      # script will use TinyUrl if URL is longer than this number.
      set etitle(tinyurl) "50"

      # Сколько трэков показывать в плйлисте радиостанций? 0 для отключения.
